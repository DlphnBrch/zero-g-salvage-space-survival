...
import hudIconsUrl from '../../drawing.png?url';

function barHTML(id, label, icon) {
  return `
    <div class="hud-bar-row">
      <div class="hud-bar-icon hud-bar-icon-${icon}" aria-hidden="true"></div>
...
export class HUD {
  constructor(gameState, onRestart) {
    ...
    this.root.id = 'hud';
    this.root.style.setProperty('--hud-icons-url', `url("${hudIconsUrl}")`);
    this.root.innerHTML = `
      <div class="hud-panel hud-left">
        <h1>ZERO-G SALVAGE</h1>
        ${barHTML('hunger-bar', 'Hunger', 'hunger')}
        ${barHTML('thirst-bar', 'Thirst', 'thirst')}
        ${barHTML('oxygen-bar', 'Oxygen', 'oxygen')}
        ${barHTML('fuel-bar', 'Extinguisher', 'fuel')}
