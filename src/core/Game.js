...
export class Game {
  ...
  const composer = new EffectComposer( this.renderer );
	...
	const colorShader = {
		uniforms: {
			tDiffuse: { value: null },
			color: { value: 1.},
			ratio: {value: this.camera.aspect}
		},
		vertexShader: `
      varying vec2 vUv;
      varying vec2 suv;
    ...
    `,
      ...
      void main() {
        ...
        gl_FragColor = vec4(previousPassColor.rgb * color, 1.0);
      }
    `,
	};
  ...
  update() {
    ...
    if(this.gameState.hunger < 10 || this.gameState.fuel < 10 || this.gameState.thirst < 10){
      this.colorPass.uniforms.color.value = (Math.sin(this.clock.getElapsedTime()*10)+3.)*0.5;
    }else{
      this.colorPass.uniforms.color.value = 1.5;
    }
    this.colorPass.uniforms.ratio.value = this.camera.aspect;
